...
namespace Lowscope.Saving
{
    ...
    public class SaveMaster : MonoBehaviour
    {
        ...
        private static bool AutoSaveOnExit
        {
            get
            {
                return SaveSettings.Get().autoSaveOnExit && !disableAutoSave;
            }
        }
        
        private static bool disableAutoSave = false;
        ...
        */
        ...
        /// <param name="id"> Add a extra identification for the scene. Useful for duplicated scenes. </param>
        /// <returns></returns>
        public static SaveInstanceManager SpawnInstanceManager(Scene scene, string id = "")
        {
            ...
            // In case a custom scene ID has been specified, we remove the scene from the multi-loaded hashset
            ...
            if (saveInstanceManagers.ContainsKey(sceneHash))
            {
                ...
                saveInstanceManagers.TryGetValue(sceneHash, out fetchInstanceManager);
                return fetchInstanceManager;
            }

            // We spawn a game object separately, so we can keep it disabled during configuration.
        ...
        public static void WriteActiveSaveToDisk(bool syncActiveSaveables = true)
        {
            ...
            else
            {
                ...
                if (Time.frameCount != 0 && !(settings.slotLoadBehaviour != SaveSettings.SlotLoadBehaviour.DontLoadSlot && AutoSaveOnExit))
                {
        ...
        public static void WipeSceneData(string name, bool clearSceneSaveables = true)
        {
            ...
            if (clearSceneSaveables)
            {
                ...
                Scene getScene = SceneManager.GetSceneByName(name);
                if (getScene.IsValid())
                {
                    SaveInstanceManager instanceManager;
                    if (saveInstanceManagers.TryGetValue(getScene.GetHashCode(), out instanceManager))
                    {
        ...
        public static void RemoveListener(Saveable saveable)
        {
            ...
            {
                if (isQuittingGame && !AutoSaveOnExit)
                    return;
        ...
        public static GameObject SpawnSavedPrefab(InstanceSource source, string filePath, string customSource = "",
            Scene scene = default(Scene), string userTag = "")
        {
        ...
        public static string ExportActiveSaveToWebJSON(bool syncSaveables = true)
        {
            if (activeSaveGame == null || activeSlot == -1)
            {
                Debug.Log("SaveMaster: failed to export active save to JSON, invalid slot or savegame loaded.");
                return "";
            }
            ...
            return SaveTransferUtility.ConvertSaveToWebJSON(activeSaveGame);
        }

        public static void OverwriteActiveSaveWithWebJSON(string json)
        {
            SaveTransferUtility.OverwriteSaveWithWebJSON(json, activeSaveGame.fileName);

            ReloadActiveSaveFromDisk();
        }
        ...
        {
            get { return instance.onStartSpawningInstances; }
            set { instance.onStartSpawningInstances = value; }
        }

        public static Action<Scene, int, int, float> OnSpawnInstanceProgress
        {
            get { return instance.onSpawnInstanceProgress; }
            set { instance.onSpawnInstanceProgress = value; }
        }
        
        public static Action<Scene> OnFinishSpawningInstances
        {
            get { return instance.onFinishSpawningInstances; }
            set { instance.onFinishSpawningInstances = value; }
        }
        ...
        private Action<Scene, int> onStartSpawningInstances = delegate { };
        private Action<Scene> onFinishSpawningInstances = delegate {  };
        private Action<Scene, SavedInstance> onSpawnedSavedInstance = delegate { };
        private Action<Scene, int, int, float> onSpawnInstanceProgress = delegate { };
        ...
        private void Awake()
        {
            ...
            if (settings.throttleOptions.disableAutoSaveDuringThrottle)
            {
                onStartSpawningInstances += StartSpawningInstances;
                onFinishSpawningInstances += FinishSpawningInstances;
            }
        }
        ...
        {
            if (throttledSpawningScenes == null)
                throttledSpawningScenes = new HashSet<Scene>();

            throttledSpawningScenes.Add(arg1);
            
            disableAutoSave = true;
        }
        private void FinishSpawningInstances(Scene obj)
        {
            if (throttledSpawningScenes.Contains(obj))
                throttledSpawningScenes.Remove(obj);

            if (throttledSpawningScenes.Count == 0)
            {
        ...
        private void Start()
        {
            ...
            // Ensures initial scenes still load saved instances when having no scene reload on.
            // As SceneManager.sceneLoaded does not get called without scene reload for the first scene.

            if (UnityEditor.EditorSettings.enterPlayModeOptionsEnabled)
            {
                int options = (int)UnityEditor.EditorSettings.enterPlayModeOptions;

                if (options == 2 || options == 3)
                {
                    int totalScenesLoaded = SceneManager.sceneCount;
                    for (int i = 0; i < totalScenesLoaded; i++)
                    {
