...
namespace TheVegetationEngine
{
    ...
    public class TVEPrefabConverter : EditorWindow
    {
        ...
        void DrawConversionSettings()
        {
            ...
            if (presetIndex != 0)
            {
                ...
                if (showAdvancedSettings)
                {
                    ...
                    if (outputMeshIndex == OutputMesh.VertexColors)
                    {
                        GUILayout.Space(10);
                        StyledGUI.DrawWindowCategory("Mesh Settings");
                        GUILayout.Space(10);
        ...
        void ConvertMeshes()
        {
            ...
            else if (outputMeshIndex == OutputMesh.VertexColors)
            {
                for (int i = 0; i < meshInstances.Count; i++)
                {
                    if (meshInstances[i] != null)
        ...
        void GetOrCopyTexture(Material material, Material instanceMaterial, string src, string dst)
        {
            ...
            //if (material.GetTexture(src) == null)
            //{
            //    return;
            //}
        ...
        void SaveMaterial(Material material)
        {
            ...
            if (File.Exists(dataPath))
            {
                ...
                EditorUtility.CopySerialized(material, materialFile);
            }
            else
            {
        ...
        void GetDescriptionFromPreset()
        {
            ...
            for (int i = 0; i < presetLines.Count; i++)
            {
                ...
                else if (presetLines[i].StartsWith("ExtraTitle"))
                {
                    infoTitle = infoTitle + " " + presetLines[i].Replace("ExtraTitle ", "");
                }
                else if (presetLines[i].StartsWith("ExtraPreset"))
                {
                    infoPreset = infoPreset + " " + presetLines[i].Replace("ExtraPreset ", "");
                }
                else if (presetLines[i].StartsWith("ExtraStatus"))
                {
                    infoStatus = infoStatus + " " + presetLines[i].Replace("ExtraStatus ", "");
                }
                else if (presetLines[i].StartsWith("ExtraWarning"))
                {
                    infoWarning = infoWarning + " " + presetLines[i].Replace("ExtraWarning ", "");
                }
                else if (presetLines[i].StartsWith("ExtraError"))
                {
                    infoError = infoError + " " + presetLines[i].Replace("ExtraError ", "");
                }
            }
        ...
        void GetOutputsFromPreset()
        {
            ...
            for (int i = 0; i < presetLines.Count; i++)
            {
                if (presetLines[i].StartsWith("OutputMesh"))
                {
                    ...
                    else if (source == "VERTEX_COLORS")
                    {
                        outputMeshIndex = OutputMesh.VertexColors;
                    }
        ...
        bool GetConditionFromLine(string line, Material material)
        {
            ...
            if (line.StartsWith("if"))
            {
                ...
                else if (type.Contains("SHADER_IS_UNITY_LIT"))
                {
                    ...
                    if (name.StartsWith("Standard") || name.StartsWith("Universal") || name.StartsWith("HDRP"))
                    {
                        valid = true;
                    }
                }
