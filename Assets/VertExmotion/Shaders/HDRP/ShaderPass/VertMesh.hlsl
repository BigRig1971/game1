...
//#endif
    ...
    //-----------------
    //VertExmotion
    float3 worldPos = GetAbsolutePositionWS(positionRWS);
    float w;
    //worldPos = VertExmotion(float4(worldPos, 1), input.color, inout normalWS, normalWS)
    //worldPos = VertExmotionBase(float4(worldPos, 1), input.color, w);
    //worldPos = VertExmotionHDRP(float4(worldPos, 1), input.color);// , inout normalWS, normalWS);

    worldPos = VertExmotionHDRP(float4(worldPos, 1), input.color, normalWS, tangentWS);
    positionRWS = GetCameraRelativePositionWS(worldPos);
    //-----------------
