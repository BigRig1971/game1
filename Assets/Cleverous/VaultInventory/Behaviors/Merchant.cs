...
namespace Cleverous.VaultInventory
{
    ...
    public class Merchant : NetworkBehaviour
    {
        ...
        {
            IUseInventory buyer = NetworkIdentity.spawned[buyerNetId].GetComponent<IUseInventory>();
            ...
            {
                Debug.Log($"<color=red>Client does not have enough currency to buy the item.</color>");
                return;
            }
        ...
        public virtual void SvrClientBuy(IUseInventory client, int merchIndex, int count)
        {
            ...
            client.Inventory.DoTake(AcceptedCurrency.Title, ItemsForSale[merchIndex].Value * count);
        ...
        {
            IUseInventory seller = NetworkIdentity.spawned[sellerNetId].GetComponent<IUseInventory>();
