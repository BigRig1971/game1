...
namespace StupidHumanGames
{
    public class CreatureMount : MonoBehaviour
    {
        ...
        [SerializeField] float mountSpeed = 3f;
        ...
        Vector3 _mountPosition = Vector3.zero;
        Rigidbody rb;
        Animator _animator;
        ...
        private void Start()
        {
            _prevCamRoot = _camRoot.localPosition;
        ...
        private void OnTriggerStay(Collider other)
        {
            if (other.CompareTag("Player"))
            {
                ...
                if (Input.GetKeyDown(KeyCode.M))
                {
                    ...
                    if (canMount)
                    {
                        _camRoot.localPosition = new Vector3(_camRoot.localPosition.x, .15f, _camRoot.localPosition.z);
                    ...
                    {
                        _camRoot.localPosition = _prevCamRoot;
                        if (!_mountUI.activeSelf) _mountUI.SetActive(true);
                        _player.transform.SetParent(null);
                        if (_animator != null) _animator.SetBool(_animation, false);
                        _player.transform.localPosition = _dismountPoint.position;
                        _tpc.OnDisableMount();
                    }
                }
                if (_player != null && canMount)
                {
                    ...
                    _player.transform.localPosition = _saddlePoint.localPosition;
                    _player.transform.localRotation = _saddlePoint.localRotation;
                    if (_input != null && _input._move != Vector2.zero) _thisAnimator.SetFloat("Blend", mountSpeed); else _thisAnimator.SetFloat("Blend", 0f);
