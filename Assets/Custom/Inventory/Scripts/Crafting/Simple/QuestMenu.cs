...
namespace StupidHumanGames
{
    public class QuestMenu : MonoBehaviour
    {
        ...
        void Awake()
        {
            quests = Resources.LoadAll<RecipeSimple>("Quests");
            foreach (RecipeSimple quest in quests)
            {
                InventorySlot slot = Instantiate(inventorySlotPrefab, recipesParent);
                slot.includeInInventory = false;
                slot.interactable = false;
                slot.SetItemInSlot(quest.result, quest.resultAmount);
                slot.GetComponent<RectTransform>().sizeDelta = Vector2.one * 45;
                slot.gameObject.AddComponent<Button>().onClick.AddListener(delegate { ShowRecipe(slot.transform, quest); });
            }
